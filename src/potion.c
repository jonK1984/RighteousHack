...
/* used to indicate whether quaff or dip has skipped an opportunity to
   ...
   any elixers [reinitialized every time it's used so does not need to
...
toggle_blindness(void)
{
    ...
    /* this vision recalculation used to be deferred until moveloop(),
       ...
       (cited case was force bolt hitting an adjacent elixer of blindness
...
/* getobj callback for object to drink from, which also does double duty as
   the callback for dipping into (both just allow elixers). */
staticfn int
drink_ok(struct obj *obj)
{
    /* getobj()'s callback to test whether hands/self is a valid "item" to
       ...
       non-inventory dungeon feature, so if there are no elixers in invent
...
dodrink(void)
{
     ...
     * force the current elixer to be unworn, and don't want to do
     * that for the entire stack when starting with more than 1.
     * [Drinking a wielded elixer of polymorph can trigger a shape
     ...
     * the current elixer is intended to keep it in inventory.]
     *
     * 3.7: switch back to relying on useup() unless the object is
     * actually worn.  Otherwise drinking a stack of unpaid elixers
...
/* elixer or spell of restore ability; for spell, otmp is a temporary
...
peffect_restore_ability(struct obj *otmp)
{
    ...
    if (otmp->cursed) {
        ...
        /* when using the elixer (not the prophecy) also restore lost levels,
           to make the elixer more worth keeping around for players with
...
peffect_water(struct obj *otmp)
{
    ...
    if (mon_hates_blessings(&gy.youmonst) /* undead or demon */
        ...
        if (otmp->blessed) {
            ...
            losehp(Maybe_Half_Phys(d(2, 6)), "elixer of holy water",
                   KILLED_BY_AN);
        } else if (otmp->cursed) {
        ...
        if (otmp->blessed) {
            ...
            if (u.ualign.type == A_LAWFUL) {
                pline("This burns like %s!", hliquid("acid"));
                losehp(Maybe_Half_Phys(d(2, 6)), "elixer of unholy water",
                       KILLED_BY_AN);
            } else
...
peffect_paralysis(struct obj *otmp)
{
    if (Free_action) {
        ...
        gm.multi_reason = "frozen by an exlier";
...
peffect_sickness(struct obj *otmp)
{
    ...
    if (otmp->blessed) {
        ...
        if (!Role_if(PM_HEALER)) {
            ...
            losehp(1, "mildly contaminated elixer", KILLED_BY_AN);
        }
    } else {
...
{
    if (otmp->cursed) {
        pline("Ulch!  That elixer tasted foul!");
        gp.potion_unkn++;
    } else if (Fixed_abil) {
        gp.potion_nothing++;
        ...
        for (ii = A_MAX; ii > 0; ii--) {
            ...
            /* only give "your X is already as high as it can get"
               message on last attempt (except blessed elixers) */
...
peffect_speed(struct obj *otmp)
{
    ...
    /* non-cursed elixer grants intrinsic speed */
...
peffect_gain_level(struct obj *otmp)
{
    ...
    pluslvl(FALSE);
    /* blessed elixers place you at a random spot in the
...
peffect_extra_healing(struct obj *otmp)
{
    ...
    exercise(A_CON, TRUE);
    exercise(A_STR, TRUE);
    /* blessed elixer also heals wounded legs unless riding (where leg
...
peffect_full_healing(struct obj *otmp)
{
    ...
    if (otmp->blessed && u.ulevel < u.ulevelmax) {
        /* when multiple levels have been lost, drinking
           multiple elixers will only get half of them back */
        u.ulevelmax -= 1;
        pluslvl(FALSE);
    }
    (void) make_hallucinated(0L, TRUE, 0L);
    exercise(A_STR, TRUE);
    exercise(A_CON, TRUE);
    /* blessed elixer heals wounded legs even when riding (so heals steed's
       legs--it's magic); uncursed elixer heals hero's legs unless riding */
...
peffect_oil(struct obj *otmp)
{
    ...
    if (otmp->lamplit) {
        if (likes_fire(gy.youmonst.data)) {
             ...
             * extra damage, but drinking elixers in that form isn't
            ...
            losehp(d(vulnerable ? 4 : 2, 4),
                   "quaffing a burning elixer of oil",
                   KILLED_BY);
        }
        /*
...
peffect_acid(struct obj *otmp)
{
    if (Acid_resistance) {
        ...
        losehp(Maybe_Half_Phys(dmg), "elixer of acid", KILLED_BY_AN);
        exercise(A_CON, FALSE);
    }
    if (Stoned)
        fix_petrification();
...
peffects(struct obj *otmp)
{
    ...
    case POT_POLYMORPH:
        ...
        break;
    default:
        impossible("What a funny elixer! (%u)", otmp->otyp);
        return 0;
    }
    return -1;
}
...
/* handle item dipped into water elixer or steed saddle splashed by same */
staticfn boolean
H2Opotion_dip(
...
/* used when blessed or cursed verse of light interacts with artifact light;
...
potionhit(struct monst *mon, struct obj *obj, int how)
{
    ...
    if (isyou) {
        ...
        losehp(Maybe_Half_Phys(rnd(2)),
               (how == POTHIT_OTHER_THROW) ? "propelled elixer" /* scatter */
                                           : "thrown elixer",
               KILLED_BY_AN);
    } else {
        tx = mon->mx, ty = mon->my;
...
potionbreathe(struct obj *obj)
{
    ...
    /* elixer of unholy water might be wielded; prevent
    ...
    /* wearing a wet towel protects both eyes and breathing, even when
       ...
       naming opportunity in case elixer was thrown at hero by a monster */
...
/* returns the elixer type when o1 is dipped in o2 */
staticfn short
...
/* call hold_another_object() to deal with a transformed elixer; its weight
...
/* #dip command - get item to dip, then get elixer to dip it into;
...
/* #altdip - #dip with "what to dip?" and "what to dip it into?" asked
   ...
   inventory item-action: the elixer has already been selected and is in
...
dip_into(void)
{
    ...
    /* "What do you want to dip into <the elixer>? [abc or ?*] " */
...
/* called by dodip() or dip_into() after obj and elixer have been chosen */
staticfn int
potion_dip(struct obj *obj, struct obj *potion)
{
    ...
    if (potion == obj && potion->quan == 1L) {
        pline("That is an exlier bottle, not a Klein bottle!");
        return ECMD_OK;
    }
    if (obj == &hands_obj) {
    ...
    if (potion->otyp == POT_WATER) {
        ...
        if (amt > (obj->odiluted ? 2 : magic ? 3 : 7)) {
            /* Trying to dip multiple elixers will usually affect only a
               subset; pick an amount between 3 and 8, inclusive, for magic
               elixer result, between 7 and N for non-magic. If diluted
               elixers are being dipped, only two are affected; this is a
               balance fix to prevent cheap mass alchemy of the (very
               common) elixer of healing into the (very valuable) elixer of
        ...
        /* [N of] the {obj(s)} mix(es) with [one of] {the elixer}... */
        ...
        if (mixture != STRANGE_OBJECT) {
            ...
            case 4:
                ...
                obj->otyp = otmp->otyp;
                /* oil uses obj->age field differently from other elixers */
    ...
    if (potion->otyp == POT_OIL) {
        ...
        if (potion->lamplit) { /* burning */
            ...
            pline_The("elixer spills and covers your %s with oil.",
            ...
            /* uses up elixer, doesn't set obj->greased */
            if (!Blind)
 ...
 more_dips:
    ...
    if ((obj->otyp == OIL_LAMP || obj->otyp == MAGIC_LAMP)
        ...
        if (obj->age > 1000L) {
            ...
            /* burns more efficiently in a lamp than in a bottle;
               diluted elixer provides less benefit but we don't attempt
    ...
    if ((obj->otyp == UNICORN_HORN || obj->otyp == AMETHYST)
        ...
        /* with multiple merged elixers, split off one and
           just clear it */
        if (potion->quan > 1L) {
        ...
        /* remove elixer from inventory and re-insert it, possibly stacking
