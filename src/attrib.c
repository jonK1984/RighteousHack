...
restore_attrib(void)
{
     ...
     * Presumably it once supported something like elixer of heroism
    ...
    int propidx) /* special cases can have negative values */
{
    ...
    if (wizard) {
        ...
        if (propidx >= 0) {
             ...
             * "from outside" (eating corpse, divine reward, blessed elixer),
             * "from experience" (from role or race at level 2+),
             * "from current form" (while polymorphed),
             * "from timed effect" (elixer or spell),
            ...
            if ((propidx == BLINDED && u.uroleplay.blind)
                ...
                Sprintf(buf, because_of,
                        ((HFast & TIMEOUT) != 0L) ? "a elixer or spell"
