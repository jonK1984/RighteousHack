    ...
    int expltype) /* explosion type: controls color of explosion glyphs */
{
        ...
        for (i = 0; i < 3; i++) {
            for (j = 0; j < 3; j++) {
                ...
                if ((explmask[i][j] & EXPL_MON) != 0) {
                     ...
                     * elixer really cold in nature, nor is damage from
                     * boiling elixer or exploding oil; only burning items
    ...
    if (uhurt) {
        /* give message for any monster-induced explosion
           or player-induced one other than verse of fire */
...
/* lit elixer of oil is exploding; extinguish it as a light source before
