    ...
    const char *verb)     /* "fall", "drop", "land", &c */
{
            ...
            if (mtmp) {
                if (!passes_walls(mtmp->data) && !throws_rocks(mtmp->data)) {
                    ...
                    /* As of 3.6.2: this was calling hmon() unconditionally
                       ...
                       dropped by a verse of earth read by a monster */
 ...
 deletedwithboulder:
        ...
        /* Potions are sometimes destroyed when landing on very hot
           ground. The basic odds are 50% for nonblessed elixers and
           30% for blessed elixers; if you have handled the object
           (i.e. it is or was yours), these odds are adjusted by Luck
           (each Luck point affects them by 2%). Artifact elixers
...
menu_drop(int retry)
{
    ...
    if (autopick) {
        /*
         * Dropping a burning elixer of oil while levitating can cause
