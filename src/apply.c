...
{
    struct obj *obj = *optr; /* obj is a elixer of oil */
    char buf[BUFSZ];
    boolean split1off;
    ...
    if (obj->lamplit) {
        You("snuff the lit elixer.");
        end_burn(obj, TRUE);
        /*
         * Free & add to re-merge elixer.  This will average the
         * age of the elixers.  Not exactly the best solution,
    ...
    You("light %selixer.%s", shk_your(buf, obj),
    ...
    if (obj->unpaid && costly_spot(u.ux, u.uy)) {
        ...
        check_unpaid(obj);
        SetVoice(shkp, 0, 80, 0);
        verbalize("That's in addition to the cost of the elixer, of course.");
        bill_dummy_object(obj);
    }
    makeknown(obj->otyp);
...
apply_ok(struct obj *obj)
{
    ...
    if (obj->oclass == POTION_CLASS) {
        /* permit applying unknown elixers, but don't suggest them */
        if (!obj->dknown || !objects[obj->otyp].oc_name_known)
            return GETOBJ_DOWNPLAY;

        /* only applicable elixer is oil, and it will only be suggested as a
