...
/* RighteousHack: helper to identify holy items that can never be cursed */
static boolean
...
mksobj_init(struct obj **obj, boolean artif)
{
    ...
    case BALL_CLASS:
        break;
    case ANOINTING_CLASS:   /* ← ADDED FOR RIGHTEOUSHACK */
    case POTION_CLASS: /* note: elixers get some additional init below */
    case SCROLL_CLASS:
...
curse(struct obj *otmp)
{
    ...
    if (is_holy_scripture(otmp))
        return;                     /* A word from God cannot be cursed */
...
{
    /* RighteousHack: A true word from God (verses, prophetic books, rods)
       can never be cursed — only blessed or neutral */
    if (is_holy_scripture(otmp)) {
        if (!otmp->blessed && !otmp->cursed && !rn2(chance) && !rn2(2))
            bless(otmp);            /* 1/4 chance to be blessed, else neutral */
        return;
    }

    /* Vanilla behavior for all other items */
...
/* elixers of oil use their obj->age field differently from other elixers
   so changing elixer type to or from oil needs to have that fixed up */
void
fixup_oil(
    struct obj *potion, /* elixer that just had its otyp changed */
    struct obj *source) /* item used to create elixer; might be Null */
{
    if (potion->otyp == POT_OIL) {
        if (source && source->otyp == POT_OIL) {
            /* elixer of oil being used to set elixer's otyp to oil;
        ...
        /* when source is a partly used oil, mark elixer as diluted */
    ...
    struct obj *targetbox) /* if non-Null, container to tip into */
{
    ...
    if (!horn || horn->otyp != HORN_OF_PLENTY) {
        ...
        if (!rn2(13)) {
            ...
            what = (obj->quan > 1L) ? "Some elixers" : "An elixer";
        } else {
...
sanity_check_worn(struct obj *obj)
{
    ...
    if (n == 1 && (carried(obj) || (owornmask & (W_BALL | W_CHAIN)) != 0L
        ...
        if (what) {
            ...
            /* if we've found a elixer worn in the amulet slot,
               this yields "worn (elixer amulet)" */
