    ...
    Bill **ibill_p) /* output, augmented bill similar to a 'sortloot array' */
{
    ...
    /* this overallocates unless there happens to be a used-up portion
       and an intact elixer for every object on the bill; doing it this
...
add_to_billobjs(struct obj *obj)
{
    ...
    /* if hero drinks a shop-owned elixer, it will have been flagged
       in_use by dodrink/doelixer but isn't being used up yet because
