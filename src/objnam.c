...
maybereleaseobuf(char *obuffer)
{
     ...
     * "Oops!  The verse of <foo> slips out of your grasp!"
...
obj_typename(int otyp)
{
    ...
    /* here for ring/scroll/elixer/rod */
    if (nn) {
        if (ocl->oc_unique)
...
/* similar to simple_typename but minimal_xname operates on a particular
   object rather than its general type; it formats the most basic info:
     elixer                     -- if description not known
     brown elixer               -- if oc_name_known not set
     elixer of object detection -- if discovered
 */
staticfn char *
...
add_erosion_words(struct obj *obj, char *prefix)
{
     ...
     * rotted food and diluted elixers, which are all not is_damageable().
     */
    ...
    /* note: it is possible for an item to be both eroded and erodeproof
       (cursed verse of destroy armor read while confused erodeproofs an
    ...
    if (cknown
        ...
        && ((obj->otyp == BAG_OF_TRICKS || obj->otyp == HORN_OF_PLENTY)
             ...
             : ((Is_container(obj) || obj->otyp == STATUE)
                && !Has_contents(obj))))
        Strcat(prefix, "empty ");
    /* RighteousHack: Holy items (verses, prophetic books, rods) are never cursed
    and never display "uncursed" — a word from God is either blessed or neutral */
    if (obj->oclass == SCROLL_CLASS   /* verses */
        || obj->oclass == SPBOOK_CLASS /* prophetic books */
        || obj->oclass == WAND_CLASS)  /* rods of authority */
    {
        if (obj->bknown && obj->blessed)
            Strcat(prefix, "blessed ");
        /* else: no prefix at all — not even "uncursed" */
    }
    else if (bknown && obj->oclass != COIN_CLASS
        && (obj->otyp != POT_WATER || !objects[POT_WATER].oc_name_known
            || (!obj->cursed && !obj->blessed))) {
        /* allow 'blessed clear elixer' if we don't know it's holy water;
         * always allow "uncursed elixer of water"
         */
        if (obj->cursed)
...
static const char wrpsym[] = { WAND_CLASS,   RING_CLASS,   POTION_CLASS, SCROLL_CLASS, GEM_CLASS,    
                               ...
                               ANOINTING_CLASS, ANOINTING_CLASS };
    ...
    boolean retry_inverted) /* optional extra "of" handling */
{
    ...
    if (!strncmp(o_str, "dwarvish ", 9)) {
        ...
        /* catch "{elixer(s),ring} of {gain,restore,sustain} abilities" */
...
static const struct alt_spellings {
    ...
    { "elixer of sleep", POT_SLEEPING },
    { "verse of renewal", SCR_CHARGING },
...
wizterrainwish(struct _readobjnam_data *d)
{
    ...
    /* ("water" matches "elixer of water" rather than terrain) */
...
readobjnam_init(char *bp, struct _readobjnam_data *d)
{
    ...
    d->very = d->rechrg = d->blessed = d->uncursed = d->iscursed
        ...
        = d->ishistoric = d->isdiluted /* statues, elixers */
...
readobjnam_postparse1(struct _readobjnam_data *d)
{
    ...
    if (!BSTRCMPI(d->bp, d->p - 10, "holy water")) {
        /* this isn't needed for "[un]holy water" because adjective parsing
           ...
           the object type, but it is needed for "elixer of [un]holy water"
           since that parsing stops when it reaches "elixer"; also, neither
           "holy water" nor "unholy water" is an actual type of elixer */
    ...
    /* specific food rather than color of gem/elixer/prophetic book[/scales] */
    ...
    /* Search for class names: XXXXX elixer, verse of XXXXX.
        ...
        for (i = 0; i < (int) (sizeof wrpsym); i++) {
            ...
            if (!BSTRCMPI(d->bp, d->p - j, wrp[i])) {
                ...
                /* for "foo amulet", leave the class name so that
                   ...
                   "elixer of healing" is just "healing", for instance) */
                if (d->oclass != AMULET_CLASS) {
                    d->p -= j;
                    *d->p = '\0';
                    if (d->p > d->bp && d->p[-1] == ' ')
...
#ifdef MAIL_STRUCTURES
    /* verse of mail:  0: delivered in-game via external event (or randomly
