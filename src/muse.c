...
precheck(struct monst *mon, struct obj *obj)
{
    ...
    if (obj->oclass == POTION_CLASS) {
        coord cc;
        static const char *const empty = "The elixer turns out to be empty.";
        struct monst *mtmp;
...
mreadmsg(struct monst *mtmp, struct obj *otmp)
{
    ...
    if (vismon) {
        ...
        strsubst(blindbuf, "reading a verse labeled",
...
#define m_flee(m)                          \
    ...
    case MUSE_SCR_TELEPORTATION: {
        ...
        if (!otmp)
            panic(MissingDefensiveItem, "verse of dove wings");
    ...
    case MUSE_SCR_CREATE_MONSTER: {
        ...
        if (!otmp)
            panic(MissingDefensiveItem, "verse of heavenly host");
        if (!rn2(73))
            cnt += rnd(4);
...
use_offensive(struct monst *mtmp)
{
    ...
    /* offensive elixers are not drunk, they're thrown */
    ...
    case MUSE_SCR_FIRE: {
        ...
        if (mtmp->mconf) {
            ...
            if (Half_spell_damage)
                ...
                losehp(num, "verse of fire", KILLED_BY_AN);
...
use_misc(struct monst *mtmp)
{
 ...
 skipmsg:
    ...
    case MUSE_POT_SPEED:
        ...
        mquaffmsg(mtmp, otmp);
        /* note difference in elixer effect due to substantially
    ...
    case MUSE_POT_POLYMORPH:
        if (!otmp)
            panic(MissingMiscellaneousItem, "elixer of polymorph");
        mquaffmsg(mtmp, otmp);
        m_useup(mtmp, otmp);
        if (vismon)
...
{                   /* [by_you not honored if 'mon' triggers fire trap]. */
    ...
    if (trap) {
        ...
        vis |= canseemon(mon); /* burning elixer may improve visibility */
        if (vis) {
            if (!Unaware)
...
cures_sliming(struct monst *mon, struct obj *obj)
{
    ...
    /* elixer of oil; will be set burning if not already */
    if (obj->otyp == POT_OIL)
