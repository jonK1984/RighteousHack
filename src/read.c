...
/* also called from teleport.c for verse of teleportation */
void
...
doread(void)
{
     ...
     * free hands because that would cripple verse of remove curse,
    ...
    if (otyp != SCR_BLANK_PAPER) {
        ...
        if (Blind)
            pline(nodisappear
                      ? "You %s the words on the verse."
                      : "As you %s the words on it, the verse disappears.",
                  silently ? "cogitate" : "pronounce");
        else
            pline(nodisappear ? "You read the verse."
                              : "As you read the verse, it disappears.");
        if (confused) {
            if (Hallucination)
                pline("Being so trippy, you have a hard time thinking straight...");
            else
                pline("Being confused, you %s the verse...",
                      silently ? "misquote" : "mispronounce");
        }
    }
    if (!seffects(scroll)) {
...
/*
 * Forget some things (e.g. after reading a verse of amnesia).  When called,
...
/* monster is hit by verse of taming's effect */
staticfn int
...
seffect_genocide(struct obj **sobjp)
{
    ...
    if (!already_known)
        You("have found a verse of judgement to the wicked!");
    gk.known = TRUE;
    if (sblessed)
        do_class_genocide();
    else
    ...
    *sobjp = 0; /* it's gone */
    ...
    if (confused) {
        if (Underwater) {
            ...
            pline_The("verse catches fire and you burn your %s.",
                      makeplural(body_part(HAND)));
            losehp(1, "verse of consuming fire", KILLED_BY_AN);
        }
        return;
    }
    if (Underwater) {
        ...
        if (sblessed) {
            if (!already_known)
                pline("This is a verse of comsuming fire!");
            dam *= 5;
        ...
        if (u_at(cc.x, cc.y)) {
            pline_The("verse erupts in a tower of flame!");
...
seffect_stinking_cloud(struct obj **sobjp)
{
    ...
    if (!already_known)
        You("have found a verse of fire and brimstone!");
    gk.known = TRUE;
...
{
    if (Blind)
        You("don't remember there being any words on this verse.");
    else
        pline("This verse seems to be blank.");
    gk.known = TRUE;
}
...
seffect_identify(struct obj **sobjp)
{
    ...
    if (is_scroll) { /* verse of discernment */
        /* known = TRUE; -- handled inline here */
        ...
        *sobjp = 0; /* it's gone */
        ...
        if (confused || (scursed && !already_known))
            You("identify this as a verse of discernment.");
        else if (!already_known)
            pline("This is a verse of discernment.");
        if (!already_known)
    ...
    boolean skip_uswallow)
{
    ...
    if (dmg)
        losehp(Maybe_Half_Phys(dmg), "verse of shaken earth", KILLED_BY_AN);
}
    ...
    struct obj *obj) /* scroll, prophetic book (for spell), or wand of light */
{
    ...
    if (!on) {
        ...
        /* verse of light becomes discovered when not blind, so some
...
do_class_genocide(void)
{
    ...
    for (j = 0; ; j++) {
        ...
        if (gameover || u.uhp == -1) {
            ...
            Strcpy(svk.killer.name, "verse of judgement to the wicked");
            if (gameover)
                done(GENOCIDED);
        }
        return;
