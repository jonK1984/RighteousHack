...
loot_classify(Loot *sort_item, struct obj *obj)
{
    ...
    /*Righteous Hack Added Anointing Oil class*/
    static char def_srt_order[MAXOCLASSES] = {
        COIN_CLASS, AMULET_CLASS, RING_CLASS, WAND_CLASS, POTION_CLASS, ANOINTING_CLASS,
...
loot_xname(struct obj *obj)
{
    ...
    /* suppress "diluted" for elixers and "holy/unholy" for water;
...
sortloot_cmp(const genericptr vptr1, const genericptr vptr2)
{
    ...
    if ((gs.sortlootmode & (SORTLOOT_PACK | SORTLOOT_INVLET))
        ...
        if ((gs.sortlootmode & SORTLOOT_INVLET) == 0) {
             ...
             * 1) elixer
             * 2) pink elixer
             * 3) dark green elixer called confusion
             * 4) elixer of healing
    ...
    const char *word)    /* name of the action */
{
 ...
 nextclass:
     ...
     * (dropping a burning elixer of oil while levitating creates
...
itemactions_pushkeys(struct obj *otmp, int act)
{
    ...
    case IA_APPLY_OBJ:
        ...
        break;
    case IA_DIP_OBJ:
        /* #altdip instead of normal #dip - takes elixer to dip into
    ...
    case IA_QUAFF_OBJ:
        /* start with m-prefix; for #quaff, it means ignore fountain
           or sink if present and drink a elixer from invent */
        cmdq_add_ec(CQ_CANNED, do_reqmenu);
...
itemactions(struct obj *otmp)
{
    ...
    if (otmp->oclass == COIN_CLASS)
        ...
        Sprintf(buf, "Dip something into %s elixer%s",
    ...
    if (otmp->oclass == POTION_CLASS) {
        Sprintf(buf, "Quaff (drink) %s",
                (otmp->quan > 1L) ? "one of these elixers" : "this elixer");
    ...
    struct obj *obj)  /* 'combine' stack */
{
    ...
    /* burning elixers of oil never merge */
...
{
    /*
     * [Should this also include lit elixers of oil?  They're not tools
 ...
 */
/*RIGHTEOUS_HACK added "Anointing Oil"*/
static NEARDATA const char *names[] = {
    0, "Illegal objects", "Weapons", "Armor", "Rings", "Amulets", "Tools",
    "Comestibles", "Elixers", "Verses", "Prophetic books", "Rods", "Coins",
    "Gems/Stones", "Boulders/Statues", "Iron balls", "Chains", "Venoms", "Anointing Oils"
};
