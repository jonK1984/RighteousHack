    ...
    if (u.ulevel > 1) {
        ...
        if (u.ulevel > 1)
            /* can happen during debug fuzzing if fuzzer_savelife() uses
               a blessed elixer of restore ability to restore lost levels */
            return;
        u.uexp = 0;
...
pluslvl(
    boolean incr) /* True: incremental experience growth;
                   * False: elixer of gain level or wraith corpse
...
long
rndexp(boolean gaining) /* gaining XP via elixer vs setting XP for polyself */
{
    ...
    /* 3.4.1:  if already at level 30, add to current experience
       ...
       level; otherwise blessed elixers of gain level can result
       in lowering the experience points instead of raising them */
    if (u.ulevel == MAXULEV && gaining) {
        result += (u.uexp - minexp);
        /* avoid wrapping (over 400 blessed elixers needed for that...) */
        if (result < u.uexp)
            result = u.uexp;
    }
    return result;
