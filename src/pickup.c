...
allow_category(struct obj *obj)
{
     ...
     * So ?!B will accept blessed scrolls or elixers, and [u will
     * accept unpaid armor.  (In 3.4.3, an object was accepted by
     * this filter if it met any entry of any type, so ?!B resulted
     * in accepting all scrolls and elixers regardless of bless/curse
     * state plus all blessed non-scroll, non-elixer objects.)
     */
    ...
    boolean telekinesis) /* not picking it up directly by hand */
{
    ...
    if (obj == uchain) { /* do not pick up attached chain */
        ...
        if (obj->blessed) {
            ...
            pline_The("verse%s %s to dust as you %s %s up.", plur(obj->quan),
...
dotip(void)
{
    ...
    if (Is_candle(cobj) && cobj->lamplit) {
        ...
        spillage = "oil";
        /* todo: reduce elixer's remaining burn timer or oil lamp's fuel */
    ...
    if (cobj->oclass == POTION_CLASS) /* can't pour elixers... */
